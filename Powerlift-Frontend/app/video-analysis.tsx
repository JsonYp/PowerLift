...
export default function VideoAnalysisScreen() {
  ...
  // State for workout date selection
  const [workoutDate, setWorkoutDate] = useState<string>(() => {
    // Always use the current date as default (local timezone)
    ...
    try {
      ...
      router.replace({
        ...
        params: {
          ...
          bodyWeight,
          weightUsed,
          workoutDate, // Pass the selected workout date
    ...
    <SafeAreaView style={dynamicStyles.container}>
      ...
      <View style={dynamicStyles.infoContainer}>
        ...
        <View style={dynamicStyles.dateContainer}>
          <Text style={dynamicStyles.infoText}>Workout Date:</Text>
          <TouchableOpacity 
              ...
              Alert.alert(
                ...
                [
                  { 
                    ...
                    onPress: () => setWorkoutDate(todayString) 
                  },
                  { 
                    text: 'Custom Date...', 
                    onPress: () => {
                      setCustomDateInput(workoutDate);
          ...
          >
            <Text style={dynamicStyles.dateButtonText}>{workoutDate}</Text>
            <Ionicons name="calendar" size={16} color={theme.text} style={{ marginLeft: 8 }} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        <View style={dynamicStyles.modalOverlay}>
          <View style={dynamicStyles.customDateModalContent}>
            <Text style={dynamicStyles.customDateTitle}>Enter Workout Date</Text>
            <TextInput
              style={dynamicStyles.customDateInput}
              ...
              autoFocus={true}
            />
            <View style={dynamicStyles.customDateButtonContainer}>
              <TouchableOpacity 
                style={dynamicStyles.customDateButton}
                onPress={() => {
                  setShowCustomDateInput(false);
                  setCustomDateInput('');
                }}
              >
                <Text style={dynamicStyles.customDateButtonText}>Cancel</Text>
              </TouchableOpacity>
              
              <TouchableOpacity 
                style={[dynamicStyles.customDateButton, dynamicStyles.customDateConfirmButton]}
                onPress={() => {
                  const today = new Date();
                  if (customDateInput && /^\d{4}-\d{2}-\d{2}$/.test(customDateInput)) {
                    const inputDate = new Date(customDateInput);
                    if (inputDate <= today && !isNaN(inputDate.getTime())) {
                      setWorkoutDate(customDateInput);
                      setShowCustomDateInput(false);
                      setCustomDateInput('');
                    } else if (inputDate > today) {
                      Alert.alert('Invalid Date', 'Workout date cannot be in the future');
                    } else {
                      Alert.alert('Invalid Date', 'Please enter a valid date');
                    }
                  } else {
                    Alert.alert('Invalid Format', 'Please enter date in YYYY-MM-DD format (e.g., 2025-07-15)');
                  }
                }}
              >
                <Text style={dynamicStyles.customDateButtonText}>Confirm</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
